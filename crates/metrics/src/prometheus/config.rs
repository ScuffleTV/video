...
impl ExporterBuilder {
	...
	/// naming conventions. For example, the counter metric `request.duration`,
	/// with unit `ms` would become `request_duration_milliseconds_total`.
	///
	...
	///
	/// Metrics such as `target_info` and `otel_scope_info` are not prefixed
	/// since these have special behavior based on their name.
	...
	pub fn with_resource_selector(mut self, resource_selector: impl Into<ResourceSelector>) -> Self {
