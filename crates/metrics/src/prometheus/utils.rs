...
pub(crate) fn get_unit_suffixes(unit: &str) -> Option<Cow<'static, str>> {
	...
	// if the first part is empty, we just return the second part if it's a match
	// with known per unit e.g
	// "test/y" => "per_year"
	...
	if let Some((first, second)) = unit.split_once('/') {
		return match (
			...
			(false, Some(first_part), Some(second_part)) => Some(Cow::Owned(format!("{}_per_{}", first_part, second_part))),
			_ => None,
		};
	}
