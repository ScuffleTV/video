...
pub use channels::{ChannelData, DataConsumer, DataProducer, PublishConsumer, PublishProducer, PublishRequest};
...
mod tests {
    ...
    async fn test_basic_rtmp_clean() {
        ...
        assert_eq!(event.app_name.as_ref(), "live");
        assert_eq!(event.stream_name.as_ref(), "stream-key");

        event.response.send(true).expect("failed to send response");
